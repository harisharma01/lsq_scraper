...
class ScraperCall:
    ...
    async def fetch_leads(self, leads_api_url, session, payload_page_num, college_name, payload_data, semaphore, delay):
        ...
        try:
            async with semaphore, session.post(leads_api_url, headers= self.headers, data= json.dumps(payload_data)) as response:
                self.info_logger.info(f'Page Number:{payload_page_num}|Response:{response.status}')
    ...
    async def main(self, cd_cust_id, college_name, start_date, end_date, semaphore, proxy_list):
        ...
        try:
            ...
            async with aiohttp.ClientSession() as session:
                ...
                #fetching leads
                for payload_page_num in range(start_page, total_pages+1):
                    tasks.append(self.fetch_leads(self.leads_api, session, payload_page_num, college_name, self.payload, semaphore, delay))
                leads_date_per_page = await asyncio.gather(*tasks)
            return leads_date_per_page, summary_data, total_leads
